...
async function fetchGoogleSheetsData(sheetID, reference) {
...
async function querydb() {
    let getMpduBaseSheetData = await fetchGoogleSheetsData(spreadsheetId, 'BaseSheetCall');
    console.log("BASE DATA SHEET ::", workbookData['43 Inch Vertical'].length);
...
function aggregateStatusCountsByBranch(data) {
    ...
    data.forEach(item => {
        ...
        const key = branchCode === "WNAG" ? `${branchCode}_${branchName}` : branchCode;
    ...
    // Group WNAG by City under single key
    const finalResult = {};

    Object.values(result).forEach(entry => {
        if (entry["Branch Code"] === "WNAG") {
            if (!finalResult["WNAG"]) finalResult["WNAG"] = [];
            finalResult["WNAG"].push(entry);
        } else {
            finalResult[entry["Branch Code"]] = entry;
        }
    });
...
function getDeviceDataByBranchAndCity(workbookData, sheetKey, branchName, cityName) {
    ...
    if (matchedEntry) {
        return {
            totalDevicesCount: Number(matchedEntry['Total Devices']) || 0,
            configuredCount: Number(matchedEntry['Configured']) || 0,
            atWspOutletCount: Number(matchedEntry['At WSP / Outlet']) || 0
        };
    } else {
        return {
            totalDevicesCount: 0,
...
async function generateMTDSummaryReport() {
    ...
    let branchArray = [];
    const branchArr = [
        { 'district': 'W', 'branch': 'WMUM', 'branch-name': 'Mumbai' },
        { 'district': 'N', 'branch': 'NJPR', 'branch-name': 'Jaipur' },
        { 'district': 'S', 'branch': 'SBLR', 'branch-name': 'Bangalore' },
        { 'district': 'W', 'branch': 'WPUN', 'branch-name': 'Pune' },
        { 'district': 'E', 'branch': 'ECAL', 'branch-name': 'Kolkata' },
        { 'district': 'N', 'branch': 'NCHA', 'branch-name': 'Chandigarh' },
        { 'district': 'N', 'branch': 'NDEL', 'branch-name': 'Delhi' },
        { 'district': 'N', 'branch': 'NEUP', 'branch-name': 'Varanasi' },
        { 'district': 'S', 'branch': 'SHYD', 'branch-name': 'Hyderabad' }
    ];

    let excelArrayData = workbookData['43 Inch Vertical'];
    let excelArrayDataVerifiedDeviceList = excelArrayData.filter(d => d['Current Status'] == 'Verified & Working');
    ...
    branchArr.forEach(branch => {
        ...
        // this code use for finding count filterwise
        if (branch.branch === 'WNAG') {
            // WNAG is an array of city objects
            statusData = (statusCountsByBranch['WNAG'] || []).find(
        ...
        const totalRecord = getDeviceDataByBranchAndCity(workbookData, '43 Tracker', branch['branch'], branch['branch-name']);

        dim.push(branch['district']);
        dim.push(branch['branch']);
        dim.push(branch['branch-name']);
        dim.push(totalRecord?.totalDevicesCount);
        dim.push(totalRecord?.configuredCount);
        dim.push(totalRecord?.atWspOutletCount);
        dim.push(getStatus('Verified & Working'));
        dim.push(getStatus('Verification Pending'));
        dim.push(getStatus('Physical Damage'));
        dim.push(getStatus('Pending Complaints'));
        dim.push(getStatus('Installation Pending'));
        dim.push(getStatus('At Teckwork'));
...
async function generateMTDDetailedReport() {
    ...
    let excelArrayData = workbookData['43 Inch Vertical'].filter(d => d['Current Status'] == 'Verified & Working');
        ...
        const info = await transporter.sendMail({
            ...
            attachments: [
                {   // file on disk as an attachment
                    ...
                    path: `./MPDU REPORT TILL${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL ECAL${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL ECAL${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL EGAU${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL EGAU${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL EVIZ${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL EVIZ${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NCHA${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NCHA${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NDEL${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NDEL${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NEUP${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NEUP${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NJPR${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NJPR${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NLUC${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NLUC${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL NSAH${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL NSAH${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL SBLR${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL SBLR${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL SCHE${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL SCHE${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL SHYD${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL SHYD${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL WBHO${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL WBHO${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL WMUM${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL WMUM${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL WNAG${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL WNAG${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL WPUN${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL WPUN${current_date}.xlsx` // stream this file
                },
                {   // file on disk as an attachment
                    filename: `MPDU REPORT TILL EORI${current_date}.xlsx`,
                    path: `./MPDU REPORT TILL EORI${current_date}.xlsx` // stream this file
                },
            ]
        });
...
async function sendNDELReportEmail(params) {
    try {
        // send mail with defined transport object
        const info = await transporter.sendMail({
            from: 'reports@techworks.co.in', // sender address
            // to:'pratik@techworks.co.in',
            cc: "sriamit1979@gmail.com", // list of receivers 
            to: "reports@techworks.co.in, sumit.gupta@techworks.co.in, sandip@techworks.co.in, pratik@techworks.co.in,dhruv@techworks.co.in",
            subject: "NDEL MPDU REPORT TILL" + current_date, // Subject line
            html: `<h6>Please find the attachment.</h6>
            <p>&nbsp;</p>
            <table style="width: 420px; font-size: 10pt; font-family: Verdana, sans-serif; background: transparent !important;"
                    border="0" cellspacing="0" cellpadding="0">
...
const dispatched = [15, 15, 17, 70, 80, 40, 30, 85, 80, 25, 10, 10, 45, 14, 25, 45, 85, 5, 25, 27];
        ...
        Promise.all([
            ...
            .then(() => {
                setTimeout(() => {
                    ...
                    // XLSX.writeFile(workbookWBHO, fileNameWBHO);
                    ...
                    // XLSX.writeFile(workbookWNAG, fileNameWNAG);
                    ...
                    // XLSX.writeFile(workbookWVIZ, fileNameWVIZ);
                    ...
                    // XLSX.writeFile(workbookWHYD, fileNameWHYD);
                    ...
                    // XLSX.writeFile(workbookWBLR, fileNameWBLR);
                    ...
                    // XLSX.writeFile(workbookWCHE, fileNameWCHE);
                    ...
                    // XLSX.writeFile(workbookWJPR, fileNameWJPR);
                    ...
                    // XLSX.writeFile(workbookWMUM, fileNameWMUM);
                    ...
                    // XLSX.writeFile(workbookWPUN, fileNameWPUN);
                    ...
                    // XLSX.writeFile(workbookWLUC, fileNameWLUC);
                    ...
                    // XLSX.writeFile(workbookWEUP, fileNameWEUP);
                    ...
                    // XLSX.writeFile(workbookWCAL, fileNameWCAL);
                    ...
                    // XLSX.writeFile(workbookWGAU, fileNameWGAU);
                    ...
                    // XLSX.writeFile(workbookWSAH, fileNameWSAH);
                    ...
                    // XLSX.writeFile(workbookWCHA, fileNameWCHA);
                    ...
                    // XLSX.writeFile(workbookWDEL, fileNameWDEL);
                    ...
                    // XLSX.writeFile(workbookEORI, fileNameEORI);
                }, 3000);
